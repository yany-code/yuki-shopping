...
3. 先 `docker compose up -d` 启动 MySQL / Redis 容器( 3306 / 6379 由容器提供)，再启动应用。 安装 Maven 后执行 `mvn spring-boot:run`，或导入 IntelliJ IDEA 后运行 `YukiShoppingApplication`。
...
- `modules/*/domain`：entity + DTO + VO，后缀区分
- `modules/*/mapper`：Mybatis-Plus Mapper , XML 在 `resources/mapper`
