...
| 方法 | 路径 | 说明 | 主要表 |
|---|---|---|---|
| POST | `/orders/preview` | 按地址与请求明细试算金额（支持自选明细），不扣库存 | 读取 SKU、商品 |
...
- 请求字段：`paymentNo`、`amount`（与支付单金额一致，字符串或数字均可）、`status`（1-成功 2-失败）、`tradeNo`（第三方交易号，可选）、`sign`（签名）。
- 签名算法：`sign = MD5(paymentNo + "|" + status + "|" + amount + "|" + 密钥)`，32 位小写十六进制；金额参与签名（`toPlainString` 形式，不压缩科学计数法、不去尾零），入签 + 回调后二次比对双重保险。
- 网关密钥：经 `.env` 的 `PAY_SECRET` 注入（`application.yml` 的 `pay.secret`），与网关侧一致；真实环境不提交进仓库。
- 示例：`paymentNo=PAY20260909120000000001`、`status=1`、`amount=8999.00`、密钥 `demo-gateway-secret` 时，`sign = MD5("PAY20260909120000000001|1|8999.00|demo-gateway-secret")`。
- 失败回调（`status=2`）仅把处理中的流水置为失败，不推进订单；签名错误返回 `40300`，金额不符返回 `40900`。
- **重复投递语义**：同一笔流水的重复成功回调（含订单已推进后重放的）一律返回 `code:0`，网关视为送达即停止重推；仅签名/金额错误与订单状态异常返回非 0。
